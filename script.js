...
function pay() {
    if(cartCount > 0) {
        document.getElementById("qrTotal").innerText = cartTotal.toLocaleString() + "đ";
        
        const nganHang = "MB"; 
        const soTaiKhoan = "0988837447"; 
        const hoTen = "DOAN QUOC CUONG"; 
        const noiDung = `Shop Cuong Thanh ${Math.floor(Math.random() * 10000)}`;

        const qrUrl = `https://img.vietqr.io/image/${nganHang}-${soTaiKhoan}-compact.png?amount=${cartTotal}&addInfo=${encodeURIComponent(noiDung)}&accountName=${encodeURIComponent(hoTen)}`;
